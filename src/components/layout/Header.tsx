...
export default function Header() {
  ...
  useEffect(() => {
    ...
    fetch('/api/phone-categories?active=true')
      .then(r => r.json())
      .then(data => { if (Array.isArray(data)) setPhoneCategories(data); })
      .catch(() => {});
  }, []);
    ...
    <header className="sticky top-0 z-50 bg-white/95 dark:bg-gray-950/95 backdrop-blur border-b border-gray-200 dark:border-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            {/* Phones dropdown */}
            <div className="relative" onMouseEnter={() => setPhonesOpen(true)} onMouseLeave={() => setPhonesOpen(false)}>
              <Link href="/phones" className={cn(navLinkCls, 'flex items-center gap-1')}>
                Phones <ChevronDown size={14} className={cn('transition-transform duration-200', phonesOpen && 'rotate-180')} />
              </Link>
              {phonesOpen && phoneCategories.length > 0 && (
                <div className="absolute top-full left-0 mt-1 w-56 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl py-2 z-50">
                  {phoneCategories.map(cat => {
                    const Icon = getPhoneCategoryIcon(cat.icon);
                    return (
                      <Link key={cat._id} href={`/phones/category/${cat.slug}`}
                        className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-700 dark:hover:text-indigo-400 transition-colors">
                  ...
                  <hr className="my-1 border-gray-100 dark:border-gray-800" />
                  <Link href="/phones" className="block px-4 py-2 text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors">
                    Browse all phones →
                  </Link>
                </div>
              )}
      ...
      {menuOpen && (
        <div className="lg:hidden border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-950 px-4 py-3 space-y-0.5">
          {[{href:'/',label:'Home'},{href:'/categories',label:'Categories'},{href:'/phones',label:'Phone Finder'},{href:'/authors',label:'Authors'},{href:'/about',label:'About'},{href:'/contact',label:'Contact'},{href:'/advertise',label:'Advertise'}].map(l => (
          ...
          {phoneCategories.length > 0 && (
            <div className="pt-2 mt-1 border-t border-gray-100 dark:border-gray-800">
              <p className="px-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider">Phone Categories</p>
              {phoneCategories.map(cat => {
                const Icon = getPhoneCategoryIcon(cat.icon);
                return (
                  <Link key={cat._id} href={`/phones/category/${cat.slug}`} onClick={() => setMenuOpen(false)}
